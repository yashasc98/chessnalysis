    ...
    public void handleSync(@DestinationVariable String gameId, @Payload GameSyncRequest request, Message<?> message) {
        try {
            ...
            gameService.getGameSnapshot(gameUuid, game -> {
                webSocketNotifier.notifyGameStateSync(userDetails.userId(), gameUuid, game.getState().name(), game.getEngine().getFen(), game.getEngine().getMoveCount(), game.getClock().getSnapshot(), game.getResult(), game.getResultReason(), game.getWhitePlayerId(), game.getBlackPlayerId(), game.getEngine().getMoves());
                return null;
            });
