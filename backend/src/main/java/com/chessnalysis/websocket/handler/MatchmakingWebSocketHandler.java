package com.chessnalysis.websocket.handler;
...
public class MatchmakingWebSocketHandler {
	...
	public void handleEnterQueue(
		@Payload EnterQueueRequest request,
		@AuthenticationPrincipal CustomUserDetails userDetails
	) {
		try {
			var queueId = matchmakingService.enterQueue(
				userDetails.userId(),
				request.timeControl()
			);

			var position = matchmakingService.getQueuePosition(
				userDetails.userId(),
				request.timeControl()
			).orElse(-1);
	...
	public void handleLeaveQueue(
		@Payload LeaveQueueRequest request,
		@AuthenticationPrincipal CustomUserDetails userDetails
	) {
		try {
